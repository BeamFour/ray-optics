...
def read_file(filename) -> tuple[list, dict]:
    global _track_contents
    _track_contents = (util.Counter() if _track_contents is None
                       else _track_contents)
...
def read_obench_file(filename, **kwargs) -> tuple["OpticalModel", tuple[dict, dict]]:
    global _track_contents
    _track_contents = util.Counter()

    obench_input, obench_dict = read_file(filename, **kwargs)
        ...
        for line in input_lines:
            ...
            if surf_num > 0:
                ...
                if line[3] == '':
                    ...
                    inpt.append(glass_name)
                    inpt.append(glass_make)
                else:
